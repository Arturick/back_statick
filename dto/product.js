...
class Product {
    ...
    async getAnalyze(task1, type, article = false){
        ...
        if(article != false){
            sqlScript = `SELECT COUNT(*), brand, img, article, barcode, size_product, price, retail_price, SUM(total_price) as totalBuy , SUM(retail_price), SUM(retail_count) as countRetail, SUM(price) FROM \`analyze\` WHERE article = ${article}`
        }
        let answer = {};
